(function() {
    'use strict';
angular.module('feed')
  .controller('FeedController', function($scope, $auth, $alert, $routeParams, $rootScope, FeedService, $q, $http, $cacheFactory) {
  FeedService.getProfile().then(function(teams) {
    ...
    var promisesNBA = [];
      ...
      if (el === "Pittsburgh Steelers") {
        ...
        $scope.nfl.push(articles.query.results);
    ...
    teams.mlb.forEach(function(el) {
      ...
      if (el === "Baltimore Orioles") {
        ...
        $scope.mlb.push(articles.query.results);
    ...
    teams.nhl.forEach(function(el) {
      ...
      if (el === "Anaheim Ducks") {
        ...
        $scope.nhl.push(articles.query.results);
    ...
    teams.nba.forEach(function(el) {
      ...
      if (el === "Milwaukee Bucks") {
        ...
        $scope.nba.push(articles.query.results);
    ...
    $q.all(promisesNFL, promisesMLB, promisesNHL, promisesNBA).then(
           function(results) {
             $scope.mlb.forEach(function(el) {
               el.item.forEach(function(elm) {
                 $scope.mlbArray.push(elm);
               });
             });
             $scope.nhl.forEach(function(el) {
               el.item.forEach(function(elm) {
                 $scope.nhlArray.push(elm);
               });
             });
             $scope.nba.forEach(function(el) {
               el.item.forEach(function(elm) {
                 $scope.nbaArray.push(elm);
               });
             });
              $scope.nfl.forEach(function(el) {
                el.item.forEach(function(elm) {
